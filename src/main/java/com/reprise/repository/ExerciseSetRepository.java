...
public interface ExerciseSetRepository extends JpaRepository<ExerciseSet, Long> {
    ...
    @Query("SELECT es FROM ExerciseSet es JOIN es.workoutExercise we JOIN we.workout w WHERE w.user.id = :userId AND we.exercise.id = :exerciseId ORDER BY w.workDate DESC, es.setNumber")
